...
pub unsafe fn softfloat_roundPackToF16(mut sign: bool, mut exp: i64, mut sig: u64) -> float16_t {
    ...
    roundBits = (sig & 0xf_i32 as u64) as u8;
    if 0x1d_i32 as u32 <= exp as u32 {
        if exp < 0_i32 as i64 {
            isTiny = softfloat_detectTininess as i32 == softfloat_tininess_beforeRounding as i32
                || exp < -1_i32 as i64
                || sig.wrapping_add(roundIncrement as u64) < 0x8000_i32 as u64;
        ...
        } else if (0x1d_i32 as i64) < exp
            || 0x8000_i32 as u64 <= sig.wrapping_add(roundIncrement as u64)
        {
            ...
            uiZ = (((sign as u16 as i32) << 15_i32) + ((0x1f_i32 as u16 as i32) << 10_i32) + 0_i32
                - (roundIncrement == 0) as i32) as u64;
            current_block = 7742372919642711988;
        } else {
            current_block = 17833034027772472439;
        }
    } else {
    ...
    if current_block == 17833034027772472439 {
        ...
        if current_block == 11042950489265723346 {
            sig &= !(((roundBits as i32 ^ 8_i32 == 0) as i32 & roundNearEven as i32) as u64);
            if sig == 0 {
                exp = 0_i32 as i64;
            }
        }
        uiZ = ((((sign as u16 as i32) << 15_i32) + ((exp as u16 as i32) << 10_i32)) as u64)
            .wrapping_add(sig);
    }
    uZ.ui = uiZ as u16;
    uZ.f
